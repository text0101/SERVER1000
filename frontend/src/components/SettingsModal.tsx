...
import { X, Moon, Sun, Shield, Trash2, Monitor, RefreshCw, CheckCircle2, AlertTriangle, Server, Network, FileText, ChevronDown, ChevronUp, Search, Clock, Award, Medal, Crown, FolderOpen } from 'lucide-react';
...
import { getTokenUsage, resetTokens, setPlan, TokenUsageData, getUploadedFiles, openUploadsFolder } from '../services/backendService';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ onClose, darkMode, toggleDarkMode }) => {
    ...
    useEffect(() => {
        ...
        const loadUploadedFiles = async () => {
            const result = await getUploadedFiles(BACKEND_API_KEY);
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/70 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 space-y-8 overflow-y-auto">
                    ...
                    {/* Storage Management */}
                    <div>
                        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Storage Management</h4>
                        ...
                        <div className="mb-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="text-left">
                                        <p className="font-semibold text-slate-700 dark:text-slate-200 text-sm">Uploaded Files</p>
                                        <p className="text-xs text-slate-500 dark:text-slate-400">{uploadedFiles.length} files saved locally</p>
                                    </div>
                                </div>
                                {showUploadedFiles ? <ChevronUp className="w-5 h-5 text-slate-400" /> : <ChevronDown className="w-5 h-5 text-slate-400" />}
                            </button>
                            ...
                            {showUploadedFiles && (
                                <div className="border-t border-slate-200 dark:border-slate-700 p-4 bg-slate-50 dark:bg-slate-900/50">
                                    ...
                                    {/* Open Folder Action (Desktop) */}
                                    <div className="mb-3">
                                        <button
                                            onClick={() => openUploadsFolder(BACKEND_API_KEY)}
                                            className="w-full py-2 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-lg text-sm font-semibold hover:bg-indigo-200 dark:hover:bg-indigo-900/50 transition-colors flex items-center justify-center gap-2"
                                        >
                                            <FolderOpen className="w-4 h-4" /> Open Storage Folder
                                        </button>
                                    </div>
                                    ...
                                    <div className="max-h-60 overflow-y-auto space-y-2 pr-1">
                                        {uploadedFiles.length === 0 ? (
                                            ...
                                            uploadedFiles
                                                .filter(f => f.name.toLowerCase().includes(searchFiles.toLowerCase()))
                                                .map((file, index) => (
                                                    <div key={`${file.name}-${index}`} className="flex items-center justify-between p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700 shadow-sm">
                                                        <div className="flex items-center gap-3 overflow-hidden">
                                                            <div className="min-w-fit p-1.5 bg-slate-100 dark:bg-slate-700 rounded text-slate-500">
                                        ...
                                        {uploadedFiles.filter(f => f.name.toLowerCase().includes(searchFiles.toLowerCase())).length === 0 && uploadedFiles.length > 0 && (
