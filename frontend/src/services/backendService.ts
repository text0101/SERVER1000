...
export const getUploadedFiles = async (apiKey: string): Promise<{ success: boolean; files?: any[]; count?: number; message?: string }> => {
  try {
    const response = await fetch(`${BACKEND_API_URL}/api/uploaded-files`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${apiKey}`
      }
    ...
    return { success: false, message: 'Failed to fetch uploaded files' };
  }
};

export const openUploadsFolder = async (apiKey: string): Promise<{ success: boolean; message: string }> => {
  try {
    const response = await fetch(`${BACKEND_API_URL}/api/open-uploads-folder`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${apiKey}`
      }
